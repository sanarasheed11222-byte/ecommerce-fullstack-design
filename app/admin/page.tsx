...
export default function AdminPage() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    const url = editId ? `/api/products/${editId}` : `${process.env.NEXT_PUBLIC_API_URL}/api/products`;
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        {message && (
          <div className={`mb-6 px-4 py-3 border text-sm font-bold ${
            message.includes('deleted') || message.includes('updated') || message.includes('added')
        ...
        <div className="border border-yellow-600/30 bg-gray-900">
          <div className="p-6 border-b border-yellow-600/20">
            <h2 className="text-xl font-bold">
              All Products <span className="text-yellow-500">({products.length})</span>
            </h2>
          </div>
